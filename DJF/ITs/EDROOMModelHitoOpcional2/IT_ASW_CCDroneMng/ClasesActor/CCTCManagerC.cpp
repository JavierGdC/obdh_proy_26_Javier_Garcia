	...
	EDROOM_CTX_Top_0::TEDROOMTransitionID arrivingTrans)
{
	...
	switch (arrivingTrans)
	{
		...
		case (EDROOM_CTX_Top_0::Init):
			edroomCurrentTrans.localId= Ready_to_Standby;
		...
		switch(Msg->signal)
		{
			...
			case (SInFlight): 
				 ...
				 if (*Msg->GetPInterface() == DroneMngCtrl)
				{
					...
					//Next transition is  ToFlight
					edroomCurrentTrans.localId= ToFlight;
					edroomCurrentTrans.distanceToContext = 0;
					edroomValidMsg=true;
				 }
		...
		switch(Msg->signal)
		{
			...
			case (EDROOMIRQsignal): 
				 ...
				 if (*Msg->GetPInterface() == RxTC)
				{
					...
					//Next transition is  NoRxTC
					edroomCurrentTrans.localId= NoRxTC;
					edroomCurrentTrans.distanceToContext = 0;
					edroomValidMsg=true;
				 }
